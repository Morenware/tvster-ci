#!/bin/bash
# edited 2015.04.20 15:50 est
set -e
SCRIPT="$(readlink -e ""$0"")"

##  install dependencies and required compiling tools from standard repos
apt-get -y install build-essential checkinstall pkg-config libtool intltool libcurl4-openssl-dev libssl-dev libevent-dev
sudo sed -i 's/TRANSLATE=1/TRANSLATE=0/' /etc/checkinstallrc
#-uncomment if needed:
#sudo apt-get -y install natpmp-utils

##  download, compile and install Transmission 2.84
set +e
cd ~
sudo rm -rf transmission-2.84; rm -f transmission-2.84.tar.xz
wget https://transmission.cachefly.net/transmission-2.84.tar.xz
xz -c -d transmission-2.84.tar.xz | tar -x
cd transmission-2.84
./configure; make
sudo checkinstall -y
cd ..
sudo rm -r transmission-2.84; rm transmission-2.84.tar.xz
set -e
